...
if ismac 
    ...
    WorkFile        = 'PPPMain_2019_06v1_3.m';
...
for ff = M_F_c_range(5:end)        %M_F_c_range
    ...
    Info                            = ImportDicom(DataPath,ExpRes,FolderName);
    ExportFigs(                       FolderName,ExpRes(3))
    dos(                              ['mv *.png ',StoreFigurePath]) 
    dos(                              ['mv *.svg ',StoreFigurePath])
    [DispM2D,Settings]              = CalDispMyr2D(FolderName,'export',ExpRes(3));
    StrainM2D                       = CallStrainKroonM2D(FolderName,ExpRes(3));
    [SDNStrain,TrStrain]            = ConvertStrain(FolderName);
    t_write                         = ExportStrainFigs(FolderName,ExpRes(3));    
    D.Myr3D = CalDispMyr3D(FolderName,'export');
